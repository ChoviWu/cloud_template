...
public class RedisRepository {
    ...
    public void setExpire(final byte[] key, final byte[] value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            LOGGER.info("[redisTemplate redis] put redis  url:{} ========redis time :{} second", key, time);
            return 1L;
        });
    }
    ...
    public void setExpire(final String[] keys, final String[] values, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                LOGGER.info("[redisTemplate redis] put redis  url:{} ========redis time :{} second", keys[i], time);
            }
            return 1L;
        });
    }
    ...
    public void set(final String[] keys, final String[] values) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                byte[] bValues = serializer.serialize(values[i]);
                connection.set(bKeys, bValues);
                LOGGER.info("[redisTemplate redis] put redis  url:{}", keys[i]);
            }
            return 1L;
        });
    }
    ...
    public void set(final String key, final String value) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            byte[] values = serializer.serialize(value);
            connection.set(keys, values);
            LOGGER.info("[redisTemplate redis] put redis  url:{}", key);
            return 1L;
        });
    }
    ...
    public byte[] get(final byte[] key) {
        ...
        LOGGER.info("[redisTemplate redis] get redis  url:{} ", key);
        return result;
    }
    ...
    public <T> T get(final String key,Class<T> clazz) {
        ...
        LOGGER.info("[redisTemplate redis] get redis  url:{} ", key);
        if(resultStr!=null){
            return JsonUtils.json2Object(resultStr,clazz);
        }
        return null;
    }
    ...
    public Long leftPush(String key, String... value) {
        if(value.length==1){
            return opsForList().leftPush(key, value[0]);
        }
    ...
    /**
     * redis List数据结构 : 获取list下的范围的元素list
     * @param key
     * @param l
     * @param size
     * @return
     */
    public List<String> range(String key,long l,long size) {
        return opsForList().range(key,l,size);
    }
    /**
     * redis List数据结构 : 返回list的Size
     *
     * @param key the key
     * @return the string
     */
